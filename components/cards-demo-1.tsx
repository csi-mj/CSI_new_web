"use client";
import React from "react";
import { CardContainer, CardBody, CardItem } from "@/components/ui/3d-card";
import Image from "next/image";
import ExpandedView, { type TeamMember } from "@/components/ui/ExpandedView";
import { AnimatePresence } from "framer-motion";

type CardProps = {
  image?: string;
  name?: string;
  profession?: string;
  gradient?: string;
  borderColor?: string;
  url?: string;
  githubUrl?: string;
  linkedinUrl?: string;
};

export default function CardDemo({
  image = "",
  name = "Md Feroz Ahmed",
  profession = "TECH HEAD",
  gradient = "linear-gradient(135deg, rgba(20,20,20,0.9), rgba(30,30,30,.95))",
  borderColor = "transparent",
  url,
  githubUrl,
  linkedinUrl,
}: CardProps) {
  const normalize = (val?: string) => {
    if (!val) return undefined;
    const v = val.trim();
    if (!v || v === 'N/A' || v === 'NA' || v === '-' || v === '#') return undefined;
    return v;
  };
  const DEFAULT_GITHUB = 'https://github.com/orgs/csi-mj';
  const DEFAULT_LINKEDIN = 'https://www.linkedin.com/company/csi-mjcet/';
  const resolvedGithub = normalize(githubUrl) || DEFAULT_GITHUB;
  const resolvedLinkedin = normalize(linkedinUrl) || DEFAULT_LINKEDIN;
  const [imageOk, setImageOk] = React.useState(Boolean(image));
  const [expandedMember, setExpandedMember] = React.useState<TeamMember | null>(null);
  const handleClick = () => {
    const member: TeamMember = {
      id: `${name}-${image}`,
      name: name || "",
      title: profession || "",
      image: image,
      specialties: [],
      social: {
        github: resolvedGithub,
        linkedin: resolvedLinkedin,
      },
    };
    setExpandedMember(member);
  };
  return (
    <>
    <CardContainer containerClassName="w-[300px]">
      <CardBody className="w-[300px]">
        <CardItem
          translateZ={12}
          onClick={handleClick}
          className="group cursor-target relative flex flex-col w-[300px] rounded-[20px] overflow-hidden border-2 border-transparent transition-colors duration-300 cursor-pointer"
          style={{
            "--card-border": borderColor,
            background: gradient,
          } as React.CSSProperties}
        >
          {/* image box (fixed height to avoid GIF resizing the card) */}
          <div className="relative z-10 h-88 w-full p-[10px] box-border overflow-hidden">
            {imageOk ? (
              <Image
                src={image}
                alt={name}
                fill
                sizes="(max-width: 768px) 100vw, 300px"
                className="object-cover object-top rounded-[10px]"
                onError={() => setImageOk(false)}
                unoptimized
              />
            ) : (
              <div className="absolute inset-0">
                <div className="absolute inset-0 rounded-[10px] bg-gradient-to-br from-black/80 via-neutral-900/80 to-black/80 backdrop-blur-sm border border-white/10" />
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="flex items-center justify-center w-24 h-24 rounded-full bg-white/10 border border-white/20 text-white text-xl font-semibold">
                  { (name || '')
                      .trim()
                      .split(/\s+/)
                      .filter(Boolean)
                      .slice(0,4)
                      .map((s) => s[0]?.toUpperCase())
                      .join('') || 'NA' }
                  </div>
                </div>
              </div>
            )}
          </div>
          {/* footer */}
          <footer className="relative z-10 p-4 text-white font-sans grid grid-cols-[1fr_auto]  items-center gap-x-3 gap-y-1">
            <div className="min-w-0">
              <h3 className="m-0 text-[1.05rem] font-semibold truncate"><span className="cursor-target" id="cursor">{name}</span></h3>
              <p className="m-0 text-[0.85rem] opacity-85 truncate"><span className="cursor-target" id="cursor">{profession}</span></p>
            </div>
            <ul className="social-wrapper">

              <li className="icon github cursor-target h-10 w-10" id="cursor">
                <span className="tooltip">Github</span>
                <a href={resolvedGithub} target="_blank" rel="noreferrer noopener" aria-label="GitHub" className="text-white/80 hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                  </svg>
                </a>
              </li>


              <li className="icon linkedin cursor-target h-10 w-10 " id="cursor">
                <span className="tooltip">LinkedIn</span>
                <a href={resolvedLinkedin} target="_blank" rel="noreferrer noopener" aria-label="LinkedIn" className="text-white/80 hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                  </svg>
                </a>
              </li>

            </ul>
          </footer>
          {/* removed gray overlay effects */}
        </CardItem>
      </CardBody>
    </CardContainer>
    <AnimatePresence mode="wait" initial={false}>
      {expandedMember && (
        <ExpandedView key={expandedMember.id} expandedMember={expandedMember} onClose={() => setExpandedMember(null)} />
      )}
    </AnimatePresence>
    </>
  );
}
