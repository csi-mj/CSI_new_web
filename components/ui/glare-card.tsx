...
export const GlareCard = ({
  ...
  return (
    <div
      ...
      ref={refElement}
      onPointerMove={(event) => {
        const rotateFactor = 0.15; // Reduced from 0.4 to 0.15 for subtler effect
    ...
    >
      <div
        className={cn(
          "h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 overflow-hidden",
          !disableFoil && "hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none"
        )}
      >
